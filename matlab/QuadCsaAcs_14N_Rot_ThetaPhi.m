...
Nptx = 2048; freq1D=zeros(Nptx,1); freq2D=zeros(Nptx,1); XX=zeros(Nptx,1); freq=zeros(3,1);
...
nphi = 800;
  ...
  dtheta = 1./double(ntheta);
  dphi = 2*pi/double(nphi);
  

%%%%%% Select the spin quantum number 
              Ispin = 1.;               % spin quantum number (3/2, 5/2, or 7/2)             
%%%%%% Adjust the spectral window              
              SW = 5000; SW3 = 200;
%%%%%% choose to consider whether a spinning or a static case %%%%%%%%%%              
              SW=SW*1000.; SW3 = SW3*1000.;
              SW2=SW/2; SW4=SW3/2;     % one half of the spectral window
%%%%%% Adjust the line broadening factor %%%%%%%
              nbroad = 41; fac=zeros(nbroad,1);  % Line broadening factors
              wlb = 0.7;    % (0 <= wlb <= 1)
              fac(1)=1.;
              for ii=1:nbroad-1
                  fac(ii+1)=fac(ii)*wlb;
...
% transformations into different coordinate frames
 ...
 U=Rabc(alpha,beta,gama); QXG=U*QX*U'; CsaQXG=U*CsaQX*U'; AcsQXG=U*AcsQX*U'; CSQXG=U*CSQX*U';             
    ...
    ct = 0.;
 for ii=1:ntheta
   st = sqrt(1-ct*ct); s2t=2*ct*st; c2t=ct*ct-st*st;
    phi = 0.;
   for jj=1:nphi
     ...
     freq(1) = Siso+sqrt(2/3)*R20cs+sqrt(1.5)*R20Q+0.5*(R2m2Q*R2p2Q-R2m1Q*R2p1Q)/wX+3*(qcsa2+qacs2); 
     freq(2) = Siso+sqrt(2/3)*R20cs-sqrt(1.5)*R20Q+0.5*(R2m2Q*R2p2Q-R2m1Q*R2p1Q)/wX-3*(qcsa2+qacs2);   
     freq(3) = freq(1)+freq(2);
    
    % Frequency position encoding (Provide a wider SW if a given SW is too narrow)
    for ijkl=1:2
           if (freq(ijkl) >= -SW2) & (freq(ijkl) <= SW2)
               frqindex = round (0.5*(freq(ijkl) + SW2)*Nptx/SW2);
           end
           
           freq1D(frqindex) = freq1D(frqindex) +fac(1);
           for ii=1:nbroad-1
               freq1D(frqindex+ii)=freq1D(frqindex+ii) +fac(ii+1);
               if frqindex-ii > 0
                freq1D(frqindex-ii)=freq1D(frqindex-ii) +fac(ii+1);
               end
           end
           ...
           if (freq(3) >= -SW4) & (freq(3) <= SW4)
               frqindex = round (0.5*(freq(3) + SW4)*Nptx/SW4);
           end
           
           freq2D(frqindex) = freq2D(frqindex) +fac(1);
           for ii=1:nbroad-1
               freq2D(frqindex+ii)=freq2D(frqindex+ii) +fac(ii+1);
               if frqindex-ii > 0
                freq2D(frqindex-ii)=freq2D(frqindex-ii) +fac(ii+1);
               end
           end
