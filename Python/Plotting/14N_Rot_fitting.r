...
U = Rabc(a,b,c) 
CsaQ = U%*%Csa%*%solve(U); AcsQ = U%*%Acs%*%solve(U); CSQ = U%*%CSPAS%*%solve(U); #check matrix multiplication here
U = Rabc(zeta, lamda, nu) 
QX = U%*%QPAS%*%solve(U); CsaQX = U%*%CsaQ%*%solve(U); AcsQX = U%*%AcsQ%*%solve(U); #check matrix multiplication here
U = Rabc(alpha, beta, gama) 
QXG = U%*%QX%*%solve(U); CsaQXG = U%*%CsaQX%*%solve(U); AcsQXG = U%*%AcsQX%*%solve(U); #check matrix multiplication here
